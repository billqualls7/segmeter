...
if __name__ == "__main__":
    ...
    cls_weights = np.array([0.1, 1, 1], np.float32)
    #------------------------------------------------------------------#
    ...
    # model = SegNext(num_classes=3, in_channnels=3, embed_dims=[32, 64, 460, 256],
    #             ffn_ratios=[4, 4, 4, 4], depths=[3, 3, 5, 2], num_stages=4,
    #             dec_outChannels=256, drop_path=2e-1)
    
    model = UNetV3_2(out_channels = num_classes)
    ...
    if local_rank == 0:
        show_config(
            ...
            save_period = save_period, save_dir = save_dir, num_workers = num_workers, num_train = num_train, num_val = num_val, flops = flops/1e6, params = params/1e6
        )
        #---------------------------------------------------------#
        #   总训练世代指的是遍历全部数据的总次数
        #   总训练步长指的是梯度下降的总次数 
    ...
    if True:
        ...
        # gen_val         = DataLoader(val_dataset  , shuffle = shuffle, batch_size = batch_size, num_workers = num_workers, pin_memory=True, 
                                    ...
                                    # worker_init_fn=partial(worker_init_fn, rank=rank, seed=seed))
        gen = DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffle)
        gen_val = DataLoader(val_dataset, batch_size=batch_size, shuffle=shuffle)
        #----------------------#
        #   记录eval的map曲线
        #----------------------#
        if local_rank == 0:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, \
                dice_loss, focal_loss, cls_weights, num_classes, fp16, scaler, save_period, log_dir , local_rank)
