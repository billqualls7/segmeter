...
class LightHighPerfPatchEmbed(nn.Module):
    def __init__(self, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        super().__init__()
        patch_size = (patch_size, patch_size)
        
        # 主路径：深度可分离卷积 + ConvNeXt 风格
        self.proj = nn.Sequential(
            # Depthwise 卷积提取空间特征
            nn.Conv2d(in_chans, in_chans, kernel_size=patch_size, stride=stride,
        ...
        self.attn = nn.Sequential(
            ...
            nn.Conv2d(embed_dim, embed_dim // 16, 1),  # 降维
            nn.ReLU(),
            nn.Conv2d(embed_dim // 16, embed_dim, 1),  # 升维
            nn.Sigmoid()
        )
        
        # 残差连接（如果输入输出通道不同，使用 1x1 卷积调整）
        self.shortcut = nn.Conv2d(in_chans, embed_dim, kernel_size=1, stride=stride,
                                  padding=0) if in_chans != embed_dim else nn.Identity()
...
class MixVisionTransformer(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, embed_dims=[32, 64, 160, 256],
        ...
        # self.patch_embed1 = LightHighPerfPatchEmbed(patch_size=7, stride=4, in_chans=in_chans, embed_dim=embed_dims[0])
        #-----------------------------------------------#
        ...
        # self.patch_embed2 = LightHighPerfPatchEmbed(patch_size=3, stride=2, in_chans=embed_dims[0], embed_dim=embed_dims[1])
        #-----------------------------------------------#
        #   利用transformer模块进行特征提取
        #   4096, 64 => 4096, 64
        #-----------------------------------------------#
        ...
        # self.patch_embed3 = LightHighPerfPatchEmbed(patch_size=3, stride=2, in_chans=embed_dims[1], embed_dim=embed_dims[2])
        #-----------------------------------------------#
        #   利用transformer模块进行特征提取
        #   1024, 160 => 1024, 160
        #-----------------------------------------------#
        ...
        # self.patch_embed4 = LightHighPerfPatchEmbed(patch_size=3, stride=2, in_chans=embed_dims[2], embed_dim=embed_dims[3])
        #-----------------------------------------------#
        #   利用transformer模块进行特征提取
        #   256, 256 => 256, 256
        #-----------------------------------------------#
...
class mit_b0(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            ...
            # self.load_state_dict(torch.load("model_data/segformer_b0_backbone_weights.pth", weights_only=True), strict=False)
