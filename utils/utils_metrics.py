...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes=None):  
    ...
    for i in range(len(IoUs)):  # 包含背景类
        print(f"Class {i}: "
            f"mIoU: {round(IoUs[i] * 100, 2):<6}; "
            f"mPA: {round(PA_Recall[i] * 100, 2):<6}; "
            f"Accuracy: {round(Precision[i] * 100, 2):<6}; "
            f"Dice: {round(Dice[i] * 100, 2):<6}")
